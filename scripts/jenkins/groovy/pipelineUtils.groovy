...
class PipelineUtils {
        ...
        for (distSpec in buildinfo.hadoop_distributions) {
            ...
            for (dockerizedDist in DOCKERIZED_DISTRIBUTIONS) {
                if (distributionStr.startsWith(dockerizedDist)) {
                    ...
                    def hivePackagePrefix = getHivePackagePrefix(distributionName, distributionVersion)
                    distributionsToBuild += [
                        name: distributionName,
                        version: distributionVersion,
                        hivePackagePrefix: hivePackagePrefix
                    ]
                }
            }
        }
    ...
    static final String HIVE_2 = 'hive2'

    private String getHivePackagePrefix(final distributionName, final distributionVersion) {
        if (distributionName.toLowerCase() == 'hdp' && distributionVersion.startsWith('2.6')) {
            return HIVE_2
        } else {
            return HIVE
        }
